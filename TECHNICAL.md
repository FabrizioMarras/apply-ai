...
- **Responsive job list** — on desktop (`md+`) jobs render as a 6-column CSS grid table (Score · Company/Role · Location · Type · Date · Status). Each row is its own independent grid (not one shared grid with the header) with a 7th `auto`-width trailing column holding the delete button; that column has no content and zero width until the row is hovered, so the reserved space doesn't show as trailing whitespace when idle. On mobile (`< md`) jobs render as cards: score badge + company/role + status pill in a single row, with location, work type, date, and the delete button as a secondary line below. The status pill and delete button use `stopPropagation` so interacting with them doesn't open the detail panel.
- **RowDeleteButton** — a `Trash2` icon shown per row (desktop table + mobile cards), hidden via `hidden group-hover:flex group-focus-within:flex` (a `display` toggle, not `opacity`, so it doesn't reserve layout space) on desktop, always visible on mobile. Clicking it swaps to inline confirm/cancel icons (✓/✕), calling `DELETE /api/delete-job` on confirm — the same endpoint and confirm-then-call pattern as `DetailPanel`'s delete, just inline in the row instead of the slide-in panel.
...
- **Sortable headers** — all 6 desktop table headers (Score, Company/Role, Location, Type, Date, Status) are clickable and drive `sortBy`/`sortDir` state. Clicking a header sorts by that field with its default direction (`DEFAULT_SORT_DIR`: desc for score/date, asc for the rest); clicking the active header again flips direction. The active header shows an `ArrowUp`/`ArrowDown` icon. Status sorts by pipeline order (`new → applied → interviewing → offer → rejected → skipped`), not alphabetically. Mobile cards have no headers, so they keep whatever sort was last set on desktop.
...
- **Archive** — jobs idle (no status change, tracked via `updated_at`) past a per-status threshold (`ARCHIVE_STALE_DAYS` in `DashboardClient.tsx`: 60 days for `new`/`applied`/`interviewing`, 21 days for the terminal `rejected`/`offer`/`skipped` since there's no pending decision left on those) are filtered out of the main table (`activeJobs`) into a collapsed "Archive" section at the bottom of the page. Archived rows show a **Restore** button that re-PATCHes the job's current status via the existing `PATCH /api/update-status` route — a no-op for the data, but the `job_applications_updated_at` DB trigger bumps `updated_at`, resetting the staleness clock so the row reappears in the active list. No schema changes or scheduled job needed; the split is computed client-side on every render.
- **Icons** — `lucide-react` throughout: `Link2` in URL input, `Upload`/`FileText` for CV section, `Download`/`ExternalLink`/`Copy`/`Check` in detail panel, `Trash2`/`X` for delete/close, `Search` in search bar, `ChevronLeft`/`ChevronRight`/`ChevronDown`/`ChevronUp` for pagination and the archive toggle, `RefreshCw` for regenerate docs and archive restore.
